...
class rna_splicing_network(rxn.ReactionNetworkDefinition):
    ...
    def __init__(self):
        ...
        reaction_dict = {0: ['mass action', 'k_on'],  # gene switching on
                         ...
                         4: ['mass action', 'gamma']  # degradation reaction
                         }
    ...
    def PSD_analytical(self, omega):
        ...
        factor = ((alpha_on - alpha_off) ** 2) * (beta ** 2) / ((beta ** 2 + omega ** 2) * (gamma ** 2 + omega ** 2))
        psd = 2 * mean_transcription_rate / (gamma ** 2 + omega ** 2) + psd_gene_switch * factor
        return psd
